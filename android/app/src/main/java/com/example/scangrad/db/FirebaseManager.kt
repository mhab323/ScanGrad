...
class FirebaseManager(private val activity: Activity) {
    ...
    /**
     * Live version of [fetchRecentSubmissions]. Emits an updated list every time
     * a relevant Firestore doc changes (e.g. when grading completes and flips
     * a doc from PENDING to GRADED). Caller must remove() the registration on
     * onDestroyView/onStop.
     */
    fun listenRecentSubmissions(
        userId: String,
        limit: Long = 3,
        onUpdate: (List<Submission>) -> Unit,
        onFailed: (String) -> Unit
        ...
        return FirebaseFirestore.getInstance()
            ...
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
                    Log.e("FirebaseManager", "listenRecentSubmissions failed", error)
                    onFailed(error.localizedMessage ?: "Failed to listen to submissions")
                    return@addSnapshotListener
                }
                val list = snapshot?.documents
                    ?.mapNotNull { it.toObject(Submission::class.java) }
                    .orEmpty()
                onUpdate(list)
            }
    }
        ...
        FirebaseFirestore.getInstance()
            ...
            .addOnSuccessListener { snapshot ->
                ...
                if (submission != null) {
                    ...
                    onFailed("Submission not found")
                }
            }
            .addOnFailureListener { e ->
                Log.e("FirebaseManager", "fetchSubmissionById failed", e)
                onFailed(e.localizedMessage ?: "Failed to fetch submission")
            }
    }
}
